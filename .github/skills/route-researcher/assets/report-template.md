...
{Peak name} rises to **{elevation with units}** with {prominence with units} of prominence in the {region, country}. The peak is located at {latitude}, {longitude} ([Google Maps]({google_maps_coordinates_link}) | [OpenTopoMap]({opentopo_map_link})). The standard route is a **{route type}** rated **{difficulty}**.
...
**Sources:** [PeakBagger]({peakbagger_url}){, [AllTrails]({alltrails_url}) if found}{, [SummitPost]({summitpost_url}) if found}
...
{If not found:}
- **Note:** Permit information not confirmed. Check with local park service or land management agency.

#### Access Conditions

{Current access notes, seasonal closures, road conditions}
...
- [Regional Weather Services]({regional_weather_link}) - Local weather forecasts and alerts
...
{If not available:}
**Avalanche forecast not available.** Check [regional avalanche center](https://www.avalanche.org/) for current conditions.
...
2. If PeakBagger reports are mostly <100 words, PRIORITIZE AllTrails/SummitPost reports
3. AllTrails reviews are typically longer and more detailed than brief PeakBagger logs
4. Mix sources by quality, not by platform - a detailed AllTrails review should appear before a brief PeakBagger report
5. If you have AllTrails/SummitPost report URLs from Step 3I, you MUST include them here}
...
{If AllTrails reviews found in Step 3I - MANDATORY to include if extracted:}
**AllTrails:**

- **{YYYY-MM-DD}** - [{Author}]({alltrails_review_url}) - ⭐ {rating}/5
- **{YYYY-MM-DD}** - [{Author}]({alltrails_review_url}) - ⭐ {rating}/5
- **{YYYY-MM-DD}** - [{Author}]({alltrails_review_url}) - ⭐ {rating}/5
- **{YYYY-MM-DD}** - [{Author}]({alltrails_review_url}) - ⭐ {rating}/5
- **{YYYY-MM-DD}** - [{Author}]({alltrails_review_url}) - ⭐ {rating}/5

{If SummitPost reports found in Step 3I:}
**SummitPost:**

- **{YYYY-MM-DD}** - [{Report Title}]({summitpost_report_url})
- **{YYYY-MM-DD}** - [{Report Title}]({summitpost_report_url})

{Format notes:
- Sort by word count/detail level (highest first) - AllTrails reviews typically rank higher than brief PeakBagger logs
- Include 📝 word count for PeakBagger reports only (AllTrails word counts usually not available)
- Include 📍 GPX indicator if has_gpx=true (PeakBagger only)
- Include ⭐ rating for AllTrails reviews
- Link to individual report page URL (not the main trail page)
- Mix sources by quality - don't group by platform
- Limit to top 5-10 total across all sources
- If no AllTrails reviews were extracted but AllTrails URL exists, note this as a failure in Information Gaps}

### Browse All Trip Reports

{Always include PeakBagger. Include other platforms only if URLs were found during data gathering (Step 3D)}
...
- {If AllTrails URL found but <3 individual review URLs extracted: "AllTrails review extraction failed - cloudscrape.py unsuccessful. Check AllTrails manually for detailed reviews."}
- {If SummitPost URL found but <3 individual trip report URLs extracted: "SummitPost trip report extraction failed - cloudscrape.py unsuccessful. Check SummitPost manually for detailed trip reports."}
...
- {If PeakBagger reports all <100 words: "Limited detailed trip reports on PeakBagger (average {X} words). AllTrails/SummitPost sources recommended for comprehensive beta."}
