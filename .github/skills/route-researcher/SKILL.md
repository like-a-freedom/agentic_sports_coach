...
description: Research mountain peaks worldwide and generate comprehensive route beta reports
---

# Route Researcher

Research mountain peaks worldwide and generate comprehensive route beta reports combining data from multiple sources including PeakBagger, SummitPost, AllTrails, weather forecasts, avalanche conditions, and trip reports.

**Data Sources:** This skill aggregates information from specialized mountaineering websites (PeakBagger, SummitPost, AllTrails, and regional avalanche centers). The quality of the generated report depends on the availability of information on these sources. If your target peak lacks coverage on these websites, the report may contain limited details. The skill works best for well-documented peaks worldwide.
...
Examples:
- "Research Mont Blanc"
- "I'm planning to climb Matterhorn next month, can you research the route?"
- "Generate route beta for Mount Kilimanjaro"
- "Research peaks in the Himalayas"
...
```
WebSearch queries (run in parallel):
1. "{peak_name} site:alltrails.com" - AllTrails page with reviews
2. "{peak_name} site:summitpost.org" - SummitPost route page
3. "{peak_name} trip report" - General trip report search
4. "{peak_name} climbing trip report" - Climbing-specific trip reports
5. "{peak_name} hiking trip report" - Hiking-specific trip reports
```
...
- General trip report URLs (if found)
- Regional climbing forum URLs (if found)
...
#### Step 3D: Weather Forecast (Open-Meteo API)
...
- Check AQI values: 0-50 (good), 51-100 (moderate), 101-150 (unhealthy for sensitive), 151-200 (unhealthy), 201-300 (very unhealthy), 301+ (hazardous)
...
**Source 3: Regional Weather Services (Supplemental)**

Use WebSearch to find local weather services for the region:
```
WebSearch queries:
1. "{peak_name} weather forecast"
2. "{peak_name} mountain weather"
3. "{peak_name} regional weather service"
```

Use WebFetch for detailed text forecast and warnings from regional services.
...
- Weather warnings or alerts (from regional services)
...
**Error Handling:**
- If Open-Meteo API fails: Fall back to regional weather services, note reduced data quality in gaps
- If Air Quality API fails: Note in gaps, continue without AQ data
- If regional weather services fail: Continue with Open-Meteo data only
- **Always provide manual check links** for Mountain-Forecast.com and regional services even when API data retrieved
...
```bash
cd skills/route-researcher/tools
uv run python fetch_avalanche.py --region "global" --coordinates "{lat},{lon}"
```

**Expected Output:** JSON with avalanche forecast from regional avalanche center

**Error Handling:**
- Script not yet implemented: Note "Avalanche script pending - check regional avalanche center manually"
- Script fails: Note in gaps with link to regional avalanche center
...
3. "{peak_name} access conditions"
```

**Extract:**
- Trailhead names and locations
- Required permits (if any)
- Access notes (road conditions, seasonal closures, park regulations)
...
**AllTrails Trip Reports (if AllTrails URL found in Step 3C):**

If AllTrails URL was found, extract trip reports (reviews) using cloudscrape.py:

```bash
# Construct endpoint: {alltrails_url}/reviews
cd skills/route-researcher/tools
uv run python cloudscrape.py "{alltrails_url}/reviews"
```

Parse HTML to extract for each review: date, author, rating, review text URL. Target 10-15 individual reviews, prioritize recent but include variety of dates.

**Error Handling:**
- If extraction yields <5 reports: Note in "Information Gaps"
- If cloudscrape.py fails: Note with AllTrails browse link

**SummitPost Trip Reports (if SummitPost URL found in Step 3C):**

If SummitPost URL was found, extract trip reports from the route page:

```bash
# Construct endpoint: {summitpost_url}/trip-reports
cd skills/route-researcher/tools
uv run python cloudscrape.py "{summitpost_url}/trip-reports"
```
...
**AllTrails Reviews:**
- List of top 3-5 reviews (recent or detailed)
- Each with: date, author, rating, URL
...
**Error Handling:**
- **AllTrails:** If cloudscrape.py fails: Note in gaps, include AllTrails browse link only
- **AllTrails:** If HTML parsing yields <5 reviews: Note as parsing failure in gaps
- **SummitPost:** If cloudscrape.py fails: Note in gaps, include SummitPost browse link
- **SummitPost:** If HTML parsing yields <5 reports: Note as parsing failure in gaps
- If no trip reports found on successfully fetched pages: Note in gaps, include browse link
- If AllTrails/SummitPost URLs not found in Step 3C: Skip those sources
...
**AllTrails/SummitPost:** Use cloudscrape.py to fetch individual trip report pages:
```bash
cd skills/route-researcher/tools
...
- Step 3D: Weather forecast (Open-Meteo + regional services)
- Step 3E: Avalanche forecast (Python script)
...
- **cloudscrape.py** - Universal fallback for WebFetch failures, works with ANY website including:
  - Cloudflare-protected sites (SummitPost, PeakBagger)
...
- **Open-Meteo Air Quality API** for AQI forecasting (AQI scale with conditional alerts)
- Multi-source weather gathering (Open-Meteo, regional services)
...
- **High-quality trip report identification** across PeakBagger, AllTrails, and SummitPost sources

**Pending Implementation:**
- `fetch_avalanche.py` - Global avalanche data (currently using WebSearch/WebFetch as fallback)
- **Browser automation** for AllTrails and SummitPost trip report extraction (requires Playwright/Chrome)
  - Current: Some sites load content via JavaScript, cloudscrape.py may not extract
...
- **Title expansion:** "Mt" → "Mount", "St" → "Saint", "Mtn" → "Mountain"
- **Add location:** "Matterhorn, Switzerland" or "Kilimanjaro, Tanzania"
...
- **International variations:** Try different language spellings if applicable (e.g., "Mont Blanc" vs "Monte Bianco")

### Google Maps and Topographic Map Links
...
```
Example: `https://www.google.com/maps/search/?api=1&query=45.8328,6.8656` (Mont Blanc)

**OpenTopoMap (for summit coordinates):**
```
https://opentopomap.org/#map={zoom}/{latitude}/{longitude}
```
Example: `https://opentopomap.org/#map/12/45.8328/6.8656`

**Note:** Use decimal degree format for coordinates. OpenTopoMap uses zoom level in URL (10-14 works well for peaks).
...
https://www.google.com/maps/search/?api=1&query={latitude},{longitude}
```
Example: `https://www.google.com/maps/search/?api=1&query=45.8500,6.8700`

**If only trailhead name available:**
```
https://www.google.com/maps/search/?api=1&query={trailhead_name}
```
Example: `https://www.google.com/maps/search/?api=1&query=Mont+Blanc+Trailhead`
